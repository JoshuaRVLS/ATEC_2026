...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Save Task E camera frames to disk while the viewer is running.",
)
parser.add_argument(
    "--cam_dir",
    type=str,
    default="debug_cam",
    help="Output directory for saved camera frames.",
)
parser.add_argument(
    "--save_every",
    type=int,
    default=10,
    help="Save one frame every N simulation steps.",
)
...
def _to_rgb_uint8(image: torch.Tensor) -> np.ndarray:
    """Convert camera output to HWC uint8 RGB for debug dumps."""
    frame = image.detach().cpu()
    if frame.ndim != 3:
        raise ValueError(f"Expected 3D image tensor, got shape {tuple(frame.shape)}")
    if frame.shape[0] in (3, 4):
        frame = frame[:3].permute(1, 2, 0)
    elif frame.shape[-1] in (3, 4):
        frame = frame[..., :3]
    else:
        raise ValueError(f"Unsupported image tensor shape: {tuple(frame.shape)}")

    if frame.dtype != torch.uint8:
        frame = (frame.float().clamp(0.0, 1.0) * 255.0).to(torch.uint8)
    return frame.numpy()


def _save_camera_frame(camera, data_type: str, output_path: Path) -> None:
    """Persist a single camera frame as PNG."""
    from PIL import Image

    frame = _to_rgb_uint8(camera.data.output[data_type][0])
...
def main():
    ...
    for i in itertools.count():
        ...
        if args_cli.save_cam and i % max(args_cli.save_every, 1) == 0:
            _save_camera_frame(video_cam, "rgb", output_dir / "video_cam" / f"frame_{i:06d}.png")
            _save_camera_frame(ee_cam, "rgb", output_dir / "ee_camera" / f"frame_{i:06d}.png")
